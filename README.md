...
| `BACKEND_RUN_LOGS_DIR` | Backend | Directory where pipeline run logs are written (relative paths are resolved from `backend/`). | Optional | `backend/.logs/pipeline_runs` | `backend/.env` (recommended), Compose, or command-line |
